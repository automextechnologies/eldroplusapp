...
function Section({ title, children }) {
  return (
    <div>
      <p className="text-xs font-extrabold text-gray-700 uppercase tracking-widest mb-3 px-1">{title}</p>
...
function Row({ icon, label, value, onClick, danger = false, last = false }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-5 py-4 text-left transition-colors hover:bg-surface ${
    ...
    >
      ...
      <div className="flex-1">
        ...
        {value && <p className="text-xs text-gray-600 font-semibold mt-1">{value}</p>}
      </div>
      {!danger && (
...
export default function Settings() {
  ...
  return (
    <div className="min-h-screen bg-surface md:p-4">
      ...
      {/* Header */}
      <div 
        className="px-5 pt-14 pb-20 md:py-10 md:rounded-3xl relative overflow-hidden mb-6"
      ...
      >
        ...
        <h1 className="relative font-display font-extrabold text-2xl text-white">Profile</h1>
        <p className="relative text-white/60 text-sm mt-0.5">Manage your account</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="px-4 md:px-0 -mt-8 md:mt-0 mb-6">
          <div className="premium-card p-5 flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center shadow-inner-sm"
                 style={{ background: 'linear-gradient(135deg, #E84C1E 0%, #C83A0E 100%)' }}>
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 font-semibold">{user.phone}</p>
              <p className="text-xs text-[#E84C1E] font-semibold mt-1">
        ...
        <div className="px-4 md:px-0 pb-28 md:pb-8 grid grid-cols-1 md:grid-cols-2 gap-6 space-y-0">
                ...
                <div className="px-5 py-4 grid grid-cols-3 gap-4">
                  {user.heightCm && (
                    <div className="text-center">
                      ...
                      <p className="text-xs text-muted mt-0.5">cm height</p>
                    </div>
                  )}
                  {user.weightKg && (
                    <div className="text-center">
                      <p className="font-mono font-bold text-xl text-gray-900">{user.weightKg}</p>
                      <p className="text-xs text-muted mt-0.5">kg weight</p>
                    </div>
                  )}
                  {bmi && (
                    <div className="text-center">
                      <p className={`font-mono font-bold text-xl ${bmiCategory.color}`}>{bmi.toFixed(1)}</p>
                      <p className="text-xs text-muted mt-0.5">BMI · {bmiCategory.label}</p>
                    </div>
                  )}
                </div>
              </Section>
          ...
          <div className="space-y-6">
            ...
            <Section title="Notifications">
              {!supported ? (
                <div className="px-5 py-4 text-sm text-muted">Push notifications not supported.</div>
              ) : permission === 'granted' ? (
                <Row
                  icon={
        ...
        <p className="text-center text-xs text-muted pt-2 pb-6 md:pb-0">HealthX · 30-Day Challenge · v1.0</p>
      </div>
    </div>
  );
}
