...
const NAV_ITEMS = [
  {
    ...
    label: 'Dashboard',
    icon: (isActive) => (
      <svg className={`w-5 h-5 transition-colors duration-200 ${isActive ? 'text-brand-600' : 'text-gray-400'}`} fill={isActive ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive ? 0 : 2}>
...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 bg-white border-r border-border h-screen sticky top-0 shrink-0 z-40">
      {/* Brand Header */}
      ...
      {user && (
        <div className="p-4 border-t border-border bg-gray-50/50">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center font-display font-bold text-brand-700 text-sm">
          ...
          <button
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 py-2 px-3 text-xs font-bold text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors border border-red-200/40"
          >
